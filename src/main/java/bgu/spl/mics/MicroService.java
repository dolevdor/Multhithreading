...
public abstract class MicroService implements Runnable {
    ...
    private LinkedBlockingQueue<TickBroadcast> timeQueue;
 //   private LinkedBlockingQueue<TerminateBroadcast> terminatedQueue;
    ...
    public MicroService(String name) {
        this.name = name;
        callbackHashMap = new HashMap<Class<? extends Message>, Callback>();
        messageBus = messageBus.getInstance();
        terminated = false;
        messageQueue = new LinkedBlockingQueue<Message>();
        timeQueue = new LinkedBlockingQueue<TickBroadcast>();
  //      terminatedQueue = new LinkedBlockingQueue<TerminateBroadcast>();
    }
    ...
    public final void run() {
        ...
        Thread timeThread = new Thread(()->{
            while(!terminated) {
                TickBroadcast e = null;
                try {
                    e = timeQueue.take();
//                    System.out.println("Im taking a time tick " + name);
                } catch (InterruptedException interruptedException) {
                    interruptedException.printStackTrace();
                }
                callbackHashMap.get(e.getClass()).call(e);
            }
        });
        ...
        while (!terminated) {
            try {
                Message e = messageBus.awaitMessage(this);
                if (e instanceof TickBroadcast) {
                    // System.out.println("got tick: "+name);
                    timeQueue.put((TickBroadcast) e);
                }
                else if (e instanceof TerminateBroadcast){
                    callbackHashMap.get(e.getClass()).call(e);
                }
                else { // This is not a tick broadcast
                    messageQueue.put(e);
                }
